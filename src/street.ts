...
export class Lane {
	...
	public generateObstacle(): void {
		...
		const newObstacle = new Obstacle(obstacleX, this.laneId * this.laneHeight, obstacleWidth, obstacleHeight, this.direction === 1 ? -speed : speed);
	...
	public updateObstacles(): void {
		...
		this.obstacles = newObstacles.filter(obstacle => !(obstacle.x > this.laneWidth || obstacle.x < -obstacle.width));

		for (let i = 0; i < this.obstacles.length; i++) {
			for (let j = i + 1; j < this.obstacles.length; j++) {
				if (Math.abs(this.obstacles[i].x - this.obstacles[j].x) < this.obstacles[i].width) {
					if (this.obstacles[i].x > this.obstacles[j].x) {
						this.obstacles[j] = this.obstacles[j].moveObstacle(this.obstacles[i].speed);
					} else {
						this.obstacles[i] = this.obstacles[i].moveObstacle(this.obstacles[j].speed);
					}
				}
			}
		}
